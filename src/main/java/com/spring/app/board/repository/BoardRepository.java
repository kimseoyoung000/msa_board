package com.spring.app.board.repository;
...
// @Repository  
public interface BoardRepository extends JpaRepository<Board, Long> { // Long 이라고 쓴 이유는 Board 클래스에 @ID 로 사용되는 num 필트의 타입이 Long 이기 때문이다.  

 /*
	BoardRepository 를 이용해 엔티티클래스명의 테이블(tbl_board)에 SQL문장 없이 CRUD 작업이 되어진다. 
	JpaRepository 는 아래와 같은 내장된 쿼리메소드를 사용하여 CRUD 작업이 되어진다.
	
	- insert 작업 : save(엔티티 객체) 
	- select 작업 : findAll(), findAll(pageable), findById(키 타입)
